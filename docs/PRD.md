...
| 创建接待/分配/取消/编辑/删除 | create/delete/update, Companion    | Y           | N         |
...
| 接送 | 订单列表 + 车辆管理 + 搜索式分配 + 分配看板                                                  | 分配时检测容量和拼车；看板模式支持批量分配              |
| 住宿 | 订单列表 + 酒店/房间管理 + 室友分配 + 分配看板                                               | 分配时检测日期重叠；看板模式单选分配                    |
| 餐饮 | 订单列表 + 餐桌分配 + 分配看板                                                               | 分配时检测餐桌容量；看板模式支持批量分配                |
| 礼品 | 订单列表 + 发放按钮                                                                          | 发放时扣减库存 + 生成费用                               |
| 接待 | 接待人员名册 + 分配列表 + 新增/编辑/删除 + 分配看板                                          | 名册支持行内编辑/删除/分享链接；看板支持 scope 批量分配 |
...
#### 4.7.5 分配看板模式（`?view=board`）

接送标签页支持 URL 参数切换为双栏看板模式（由 `ViewToggle` 切换）：

- **左栏**：待分配订单列表（嘉宾名 + 等级 Badge + 路线 + 时间），支持多选（复选框）+ 搜索
- **右栏**：可用车辆列表（车牌 + 司机 + 车队 + 容量），支持搜索
- **交互**：勾选多个订单 → 点击右栏车辆 → 批量分配（`assignVehicle` 循环调用）。座位不足时部分成功，toast 提示「已分配 X 项，Y 项失败」
- **适用场景**：会前大批量集中分配，比单个弹窗效率更高
...
#### 4.8.5 分配看板模式（`?view=board`）

住宿标签页支持双栏看板模式：

- **左栏**：待分配订单（嘉宾名 + 入住~退房日期范围），单选模式（`selectMode='single'`，一次只选一个订单）
- **右栏**：所有房间（房号 + 酒店名 + 房型 + 状态 Badge）
- **房间状态过滤**：AVAILABLE / RESERVED 房间可点击分配；OCCUPIED / MAINTENANCE 显示为禁用灰色，仅作上下文展示
- **交互**：选中一个订单 → 点击可用房间 → `assignRoom`。房间被占用或维护中时按钮不可点击
- **适用场景**：酒店房间逐一分配，避免误选占用中的房间
...
#### 4.9.1 分配看板模式（`?view=board`）

餐饮标签页支持双栏看板模式：

- **左栏**：待分配订单（嘉宾名 + 餐类 Badge + 饮食禁忌 + 时间），多选
- **右栏**：餐桌列表（桌名 + 类型 + 容量 + 占用情况 `X/Y`）
- **满桌禁用**：`occupiedCount >= capacity` 的餐桌显示「已满」并禁用点击
- **交互**：勾选多个订单 → 点击有剩余座位的餐桌 → 批量 `assignTable`。部分超出容量时部分成功，toast 提示
- **适用场景**：多人宴会场景快速安排座位
...
- 接待人员库为全局共享（跨会议复用）
- 分配范围（assignmentScope）：全程 / 会议期间 / 用餐 / 接送 / 住宿
- 接待端门户：显示每位嘉宾的完整接待信息

#### 4.11.2 接待人员名册（CompanionRoster）

列表模式下接待标签页首部展示所有接待人员名册：

- 每行显示：头像（姓名首字）+ 姓名 + 职务 + 语言 + 手机 + 已分配嘉宾数 Badge
- **行内操作按钮**：
  - 分享：复制 `/companion/{id}` 链接到剪贴板
  - 编辑：进入行内编辑模式（姓名/职务/手机/语言），调用 `updateCompanion`
  - 删除：仅对未分配的接待人员可用，已分配的不显示删除按钮，调用 `deleteCompanion`

#### 4.11.3 分配看板模式（`?view=board`）

接待标签页支持双栏看板模式：

- **顶部 scope 选择器**：全程 / 会议期间 / 用餐 / 接送 / 住宿（批量分配时统一使用此 scope）
- **左栏**：未分配接待人员的嘉宾列表（姓名 + 单位 + 等级 Badge），多选
- **右栏**：所有接待人员（姓名 + 职务 + 语言 + 手机 + 已分配数量 Badge），多选
- **交互**：勾选多个嘉宾 → 选择 scope → 点击接待人员 → 批量 `assignCompanion`
- **适用场景**：一位接待人员负责多位嘉宾，或一位嘉宾分配给多位接待人员的快速操作

#### 4.11.4 接待端手机验证门

打开 `/companion/{id}` 时，若该接待人员档案中存在手机号，则强制弹出验证页：

- 输入框要求填入接待人员手机号后 4 位
- 验证通过（前端 `pin === phoneLastFour`）才显示任务详情
- 验证失败显示红色提示，可重试（无次数限制）
- 接待人员档案无手机号时跳过验证直接展示（向后兼容）
- **目的**：缓解 Companion ID 作为永久 token 的安全风险，即便链接泄露，攻击者仍需知道手机后 4 位

#### 4.11.5 接待端嘉宾切换卡片

接待人员负责的嘉宾可能不止一位，门户顶部提供横向嘉宾切换标签：

- 每位嘉宾一个按钮：姓名 + 等级 Badge（小号）
- 当前选中的嘉宾高亮（深色背景），其余浅色
- 仅当前嘉宾的详情卡片渲染（避免一次性渲染所有数据）
- 单嘉宾时不显示切换栏
...
1. 工作人员在接待标签页（列表模式）的接待人员名册行内点击分享图标 → 复制 `/companion/{id}` URL
2. 接待人员打开链接 → 若档案有手机号，输入后 4 位验证（验证门）
3. 验证通过后看到顶部嘉宾切换栏 + 当前嘉宾的完整接待信息
4. 点击「分享嘉宾行程给本人」→ 生成嘉宾端链接
5. 复制 → 转发给嘉宾
...
| 项目     | 说明                                                                              |
| -------- | --------------------------------------------------------------------------------- |
| 认证     | Companion ID（CUID，不可猜测但非 HMAC）+ 手机后 4 位二次验证                      |
| 验证门   | 打开后若档案有手机号，必须输入后 4 位验证才能查看任务详情（前端比对，无次数限制） |
| 页面内容 | 当前选中嘉宾的完整接待信息（档案/接送/住宿/餐饮/议程）                            |
| 嘉宾切换 | 顶部横向嘉宾卡片（姓名 + 等级 Badge），切换查看不同嘉宾，仅渲染当前一位           |
...
### 7.11 Prisma 嵌套关系加密字段解密

| 场景                                                                                     | 处理                                                                                                                                                                           |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `prisma.meetingGuest.findMany({ include: { guest: true } })` 中的 `Guest.phone` 加密字段 | Prisma `$extends` 拦截器使用**递归解密**（`decryptRecursive`），扫描返回对象的所有层级，遇到 `enc:` 前缀字段自动解密                                                           |
| 历史问题                                                                                 | 早期实现仅按 `model` 参数解密顶层字段，嵌套 include 关系中的加密字段无法被解密，导致接待端等场景读取到密文。已修复为递归扫描（`ALL_ENCRYPTED_FIELDS` 全局字段集合 + 深度遍历） |
| 影响范围                                                                                 | `lib/db/prisma-extensions.ts` 的 `decryptFieldsOn` 不再依赖 `model` 参数，所有 `find*` / `create` / `update` / `upsert` 结果均递归处理                                         |
...
| 视图切换     | ViewToggle 列表/看板双模式开关（通过 `?view=board` URL 参数控制） |
| 分配看板     | AssignmentBoard 通用组件：左栏多选/单选 + 右栏资源点击批量分配    |
...
| 接待端     | Companion ID（CUID）+ 手机后 4 位    | 无额外存储     | 永久  | 前端手机后 4 位验证门（非 HMAC） |
...
| 4   | 接待端 Token 安全较弱（已缓解） | 使用 Companion ID 而非 HMAC Token，但已加入手机后 4 位二次验证门（前端比对），降低了链接泄露的暴露面。后端 Token 机制升级为 HMAC 仍为待改进项 | 低     |
...
| 分配看板     | Assignment Board         | 双栏批量分配模式，左栏多选待分配项，右栏点击资源完成分配 |
| 视图切换     | View Toggle              | 列表/看板双模式 URL 参数切换                             |
| 接待人员名册 | Companion Roster         | 接待标签页的接待人员集中列表，含行内编辑/删除/分享       |
| 接待端验证门 | Companion Phone Verify   | 接待端打开时需输入手机后 4 位的二次验证                  |

> 术语变更：早期文档中的「陪同人员」/「陪同端」自 v3.0 起统一改为「接待人员」/「接待端」。代码中模型名（Companion / CompanionAssignment）和数据字段保持不变。
