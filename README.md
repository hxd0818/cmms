...
- **以接待运营为核心**：不是日程发布系统，而是覆盖接送/住宿/餐饮/礼品/接待全链路的运营工具
- **跨会议嘉宾沉淀**：VIP 档案长期累积，同一嘉宾参加多场会议只存一份
- **会议独立资源池**：每场会议管理自己的车辆、酒店房间、餐桌（资源标签页集中管理），互不干扰
- **字典后台可配置**：所有枚举标签集中管理，管理员改标签不需要改代码
- **四端架构**：工作人员端（PC/H5）+ 嘉宾端（H5 链接分享行程）+ 司机端（H5 链接）+ 接待端（H5 手机验证 + 嘉宾切换 + 分享链接）
- **现场实操导向**：任务看板、签到扫码、状态机驱动、拼车支持、分配看板批量分配，服务真实接待场景
- **VIP 数据安全**：手机/身份证字段加密存储、有状态 token 可吊销、字段级权限脱敏、接待端手机后 4 位二次验证
...
| v3.0 增强 | ✅ 完成                         | 分配看板（接送/住宿/餐饮/接待）/接待人员名册 CRUD/接待端手机验证门/接待端嘉宾切换卡片/分享嘉宾链接/Prisma 加密扩展嵌套解密修复 |
...
│   │   ├── guests/
│   │   └── meetings/
│   ├── guest/[token]/      # 嘉宾端（Token 分享行程）
│   ├── companion/[token]/  # 接待端（手机验证 + 嘉宾切换 + 分享链接）
│   ├── driver/[token]/     # 司机端
